...
public class TickSpeed {
	...
	@Setting("Maximum")
	private List<Max> max = Arrays.asList(new Max("minecraft:overworld", 3), new Max("minecraft:the_nether", 3), new Max("minecraft:the_end", 3));
	@Setting("Ticks")
	...
	private TicksSection ticks = new TicksSection(25d, 10d);
	...
	public int getMax(ServerWorld world) {
		return max.stream().filter(max -> max.getWorld().equals(world.key().asString())).findFirst().map(Max::getValue).orElse(max.stream().filter(max -> max.getWorld().equals("minecraft:overworld")).findFirst().map(Max::getValue).orElse(3));
	}
